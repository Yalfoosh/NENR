...
class Fuzzy:
    ...
    def fuzzy_gamma(alpha: int, beta: int):
        if alpha is None or beta is None:
            raise ValueError(DZ1.defaults.GAMMA_FUNCTION_NONE_PARAMETERS)

        if not (isinstance(alpha, int) and isinstance(beta, int)):
            raise TypeError(DZ1.defaults.GAMMA_FUNCTION_NON_INT_PARAMETERS)

        if not alpha < beta:
            raise ValueError(DZ1.defaults.GAMMA_FUNCTION_WRONG_ORDER_PARAMETERS)
    ...
    @staticmethod
    def fuzzy_l(alpha: int, beta: int):
        return lambda x: 1. if x <= alpha \
            else 0. if x >= beta \
            else (x - beta) / (alpha - beta)

    @staticmethod
    def fuzzy_lambda(alpha: int, beta: int, gamma: int):
        if alpha is None or beta is None or gamma is None:
            raise ValueError(DZ1.defaults.LAMBDA_FUNCTION_NONE_PARAMETERS)

        if not (isinstance(alpha, int) and isinstance(beta, int) and isinstance(gamma, int)):
            raise TypeError(DZ1.defaults.LAMBDA_FUNCTION_NON_INT_PARAMETERS)

        if not alpha < beta < gamma:
            raise ValueError(DZ1.defaults.LAMBDA_FUNCTION_WRONG_ORDER_PARAMETERS)
    ...
    def fuzzy_pi(alpha: int, beta: int, gamma: int, delta: int):
        if alpha is None or beta is None or gamma is None or delta is None:
            raise ValueError(DZ1.defaults.PI_FUNCTION_NONE_PARAMETERS)

        if not (isinstance(alpha, int) and isinstance(beta, int) and isinstance(gamma, int) and isinstance(delta, int)):
            raise TypeError(DZ1.defaults.PI_FUNCTION_NON_INT_PARAMETERS)

        if not alpha < beta < gamma < delta:
            raise ValueError(DZ1.defaults.PI_FUNCTION_WRONG_ORDER_PARAMETERS)
...
class MutableFuzzySet(Fuzzy):
    def __init__(self, domain: Domain):
        if domain is None:
            raise ValueError(DZ1.defaults.DOMAIN_FUZZY_SET_NONE)
