...
class Domain:
    def __init__(self, integer_range: Tuple[int, int] = DZ1.defaults.DEFAULT_DOMAIN_RANGE):
        if integer_range is None or not isinstance(integer_range, tuple) or not len(integer_range) == 2\
            ...
            integer_range = DZ1.defaults.DEFAULT_DOMAIN_RANGE
    ...
    def from_domains(domains: Tuple["Domain", "Domain"]) -> "Domain":
        if domains is None or not isinstance(domains, tuple) or not len(domains) == 2\
            ...
            raise ValueError(DZ1.defaults.DOMAIN_INPUT_IS_POOPOO)
