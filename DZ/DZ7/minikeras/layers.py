...
class Layer:
    ...
    def inject_parameters(self, parameters: np.ndarray):
        pass
...
class Dense(Layer):
    ...
    def extract_parameters(self):
        if self.trainable:
            return np.concatenate([np.reshape(self.weights, (-1, )), np.reshape(self.biases, (-1, ))])

    def inject_parameters(self, parameters: np.ndarray):
        _weights = parameters[:self.__input_units * self.__output_units]
        _biases = parameters[len(parameters) - self.__output_units:]

        self.__weights = np.reshape(_weights, self.weights.shape)
        self.__biases = np.reshape(_biases, self.__biases.shape)
...
class Distance(Layer):
    def __init__(self, output_units: int, input_units: int or None = None, name: str = "Distance",
                 activation: Activation or str = "linear", **kwargs):
    ...
    def build(self, input_units: int or None, **kwargs):
        ...
        self.__weights = np.random.normal(loc=0.0,
                                          scale=2 / np.sqrt(input_units + self.__output_units),
                                          size=(input_units, self.__output_units))
        self.__scale = np.random.uniform(size=self.__output_units)
    ...
    def forward(self, inputs: np.ndarray, **kwargs):
        divided = np.copy(self.weights)

        for i in range(len(divided)):
            divided[:, i] -= inputs

        return np.reciprocal(1. + np.divide(np.linalg.norm(divided), np.abs(self.scale)))

    def backward(self, inputs: np.ndarray, output_gradient: np.ndarray, gradient_tape: Dict):
        weight_gradient = np.copy(output_gradient)
        scale_gradient = 0

        if gradient_tape[self.id] is None:
            gradient_tape[self.id] = np.array([weight_gradient]), np.array([scale_gradient])
        else:
            gradient_tape[self.id] = np.stack([gradient_tape[self.id][0][0], weight_gradient]), \
                                     np.stack([gradient_tape[self.id][1][0], scale_gradient])
    ...
    def extract_parameters(self):
        if self.trainable:
            return np.concatenate([np.reshape(self.weights, (-1,)), np.reshape(self.scale, (-1,))])

    def inject_parameters(self, parameters: np.ndarray):
        _weights = parameters[:self.__input_units * self.__output_units]
        _scale = parameters[len(parameters) - self.__output_units:]

        self.__weights = np.reshape(_weights, self.weights.shape)
        self.__scale = np.reshape(_scale, self.__scale.shape)
